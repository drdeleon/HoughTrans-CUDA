...
*/
__global__ void GPU_HoughTransPro(unsigned char *pic, int w, int h, int *acc, float rMax, float rScale)
{
  ...
  if (pic[gloID] > 0)
    {
      for (int tIdx = 0; tIdx < degreeBins; tIdx++) {
          ...
          atomicAdd (locAcc + (rIdx * degreeBins + tIdx), 1);
        }
    }
...
int main (int argc, char **argv)
{
  ...
  //llamada de kernel
  GPU_HoughTransPro <<< blockNum, 256 >>> (d_in, w, h, d_hough, rMax, rScale);
